...
var Thread = {
	...
	{
        if($('simpleCaptcha_img'))
        {   
            simpleCaptchaData = request.responseText.match(/^<captcha>([0-9a-zA-Z]+)\|([0-9a-zA-Z]+)\|([0-9]+)\|([0-9a-zA-Z]+)\|([0-9a-zA-Z]+)<\/captcha>/);
            if(simpleCaptchaData)
            {
                request.responseText = request.responseText.replace(/^<captcha>(.*)<\/captcha>/, '');
                
                if(simpleCaptchaData[1])
                {
                    
                    var oldHash = $('specialHash').value;
                    $('imagehash').value = simpleCaptchaData[4];
                    
                    if(simpleCaptchaData[5] != '0')
                    {
                        $(oldHash).type = "hidden";
                        $(oldHash).value = simpleCaptchaData[5];
                        $('captcha_trow').style.display = "none";
                    }
                    else
                    {
                        $('simpleCaptcha_img').src = "captcha.php?action=regimage&imagehash=" + simpleCaptchaData[4];
                        $(oldHash).type = "text";
                        $(oldHash).value = "";
                        $('captcha_trow').style.display = "";
                    }
                    $(oldHash).setAttribute("name", simpleCaptchaData[2]);
                    $(oldHash).setAttribute("id", simpleCaptchaData[2]);
                    $('specialHash').value = simpleCaptchaData[2];
                }
            }
        }
